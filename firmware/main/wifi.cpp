...
namespace
{
    ...
    void smartconfig_task(void *parm)
    {
        ...
        while(true) {
            ...
            uxBits =
                xEventGroupWaitBits(internal_wifi_event_group, WIFI_CONNECTED_BIT | WIFI_ESPTOUCH_DONE_BIT, pdTRUE, pdFALSE, portMAX_DELAY);
    ...
    {
        LOG_DEBUG("WIFI EVENT: %s (%ld)", event_base, event_id);
...
esp_err_t wifi_init()
{
    ...
    LOG_INFO("prepare to init complete");
    ...
    ESP_CHECK(esp_wifi_get_config(WIFI_IF_STA, &saved_config));

    LOG_INFO("get_config complete");
    ...
    ESP_CHECK(esp_event_handler_register(WIFI_EVENT, ESP_EVENT_ANY_ID, &wifi_event_handler, NULL));
    ESP_CHECK(esp_event_handler_register(IP_EVENT, IP_EVENT_STA_GOT_IP, &wifi_event_handler, NULL));
    ESP_CHECK(esp_event_handler_register(SC_EVENT, ESP_EVENT_ANY_ID, &wifi_event_handler, NULL));
