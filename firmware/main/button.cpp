...
button_t &button_left = buttons[BUTTON_LEFT];
button_t &button_down = buttons[BUTTON_DOWN];
button_t &button_up = buttons[BUTTON_UP];
button_t &button_right = buttons[BUTTON_RIGHT];
button_t &button_select = buttons[BUTTON_SELECT];

namespace
{
    button_t real_buttons[NUM_BUTTONS] = { { .gpio_num = BUTTON_0_GPIO },
                                           { .gpio_num = BUTTON_1_GPIO },
                                           { .gpio_num = BUTTON_2_GPIO },
                                           { .gpio_num = BUTTON_3_GPIO },
                                           { .gpio_num = BUTTON_4_GPIO } };
    ...
    void read_buttons(void *arg)
    {
        for(int i = 0; i < NUM_BUTTONS; ++i) {
            button_t &button = real_buttons[i];
...
{
    for(int i = 0; i < NUM_BUTTONS; ++i) {
        buttons[i] = real_buttons[i];
        real_buttons[i].pressed = 0;
        real_buttons[i].released = 0;
    }
}
